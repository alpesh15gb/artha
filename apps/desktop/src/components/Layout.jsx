...
function Layout({ children }) {
  ...
  useEffect(() => {
    if (user && businesses.length === 0) {
      console.log('[Layout] No businesses in store, fetching...');
      fetchBusinesses().then(data => {
        console.log(`[Layout] Fetched ${data?.length || 0} businesses`);
        if ((data?.length || 0) === 0 && !location.pathname.startsWith('/onboarding')) {
           navigate('/onboarding');
        }
      }).catch(err => {
      ...
      console.log(`[Layout] User ${user.email} has ${businesses.length} businesses in store`);
    }
  }, [user]);
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc]">
      ...
      <motion.aside
        ...
        className="bg-white border-r border-gray-100 flex flex-col relative z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)] no-print"
      >
        {/* Logo Section */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        {/* Header */}
        <header className="h-20 glass sticky top-0 z-40 flex items-center justify-between px-8 mx-6 mt-4 rounded-3xl shadow-sm border border-white/40 no-print">
