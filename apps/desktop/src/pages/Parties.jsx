...
function PartyModal({ isOpen, onClose, item, businessId }) {
  ...
  const handleGstLookup = async () => {
    const gstin = formRef.current?.elements['gstin'].value;
    ...
    try {
      ...
      if (data) {
        ...
        if (formRef.current) {
          formRef.current.elements['name'].value = data.tradeName || data.legalName;
          formRef.current.elements['address'].value = `${data.address.buildingName || ''} ${data.address.street || ''}`.trim();
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
           <div className="md:col-span-2 flex items-end gap-2">
              ...
              <Button 
                ...
                className="h-[42px] mb-0.5 shadow-indigo-100"
                variant="primary"
                size="sm"
              >
                Verify Details
           ...
           <div className="md:col-span-1">
             <Select label="Party Type" name="partyType" defaultValue={item?.partyType || 'CUSTOMER'} options={[{value:'CUSTOMER', label:'Customer'}, {value:'SUPPLIER', label:'Supplier'}, {value:'BOTH', label:'Both (Cust & Supp)'}]} />
           </div>
        </div>

        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-4">
           <div className="flex items-center justify-between">
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Address & Location</p>
           </div>
           <Input label="Street Address" name="address" defaultValue={item?.address} placeholder="Door No, Building, Area" />
           <div className="grid grid-cols-3 gap-4">
             <div className="col-span-1"><Input label="City" name="city" defaultValue={item?.city} placeholder="City" /></div>
             <div className="col-span-1"><Input label="State" name="state" defaultValue={item?.state} placeholder="Enter State" /></div>
             <div className="col-span-1"><Input label="PIN Code" name="pincode" defaultValue={item?.pincode} placeholder="6-digits" /></div>
           </div>
        </div>
