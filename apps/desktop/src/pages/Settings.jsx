    ...
    try {
      ...
      if (data) {
        ...
        setSettings({
          ...
          address: {
            ...settings.address,
            street: `${data.address.buildingName || ''} ${data.address.street || ''}`.trim(),
                ...
                <motion.div key="business" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1">
                        <Input label="Business GSTIN" value={settings?.gstin || ''} onChange={e => setSettings({ ...settings, gstin: e.target.value })} placeholder="22AAAAA0000A1Z5" />
                      </div>
                      <Button 
                        type="button" 
                        onClick={handleGstLookup} 
                        loading={isFetchingGst}
                        className="h-[42px] mb-0.5"
                        variant="secondary"
                        size="sm"
                      >
                        Auto-Fill
