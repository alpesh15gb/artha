...
const metrics = useMemo(() => {
  ...
  // Force black text for printing/export by toggling a body class
  React.useEffect(() => {
  ...
  const handlePrint = async (inv) => {
    ...
    // Auto-close after print dialog opens (approximation)
    setTimeout(() => {
      setPreviewInvoice(null);
    }, 10000);
  };
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
             <span className="text-sm font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">All Invoices ({invoices.length})</span>
             <span className="w-1 h-1 bg-gray-300 rounded-full" />
             <span className="text-sm text-gray-500">Draft ({metrics.DRAFT || 0})</span>
          </div>
        </div>
      ...
      {/* Generation Overlay */}
      {isExporting && (
        <div className="fixed inset-0 z-[10001] bg-slate-900/60 backdrop-blur-md flex items-center justify-center no-print">
          <div className="bg-white p-10 rounded-[40px] shadow-2xl flex flex-col items-center gap-6 scale-110">
             <div className="w-16 h-16 border-[6px] border-indigo-600 border-t-transparent rounded-full animate-spin" />
             <div className="text-center">
                <p className="font-black text-2xl text-gray-900 tracking-tight">Creating Document</p>
                <p className="text-gray-500 font-bold mt-1 uppercase text-[10px] tracking-widest">Optimizing high-resolution capture...</p>
             </div>
          </div>
        </div>
      )}

      {/* Hidden Preview Area for Export/Print */}
      {previewInvoice && (
        <Portal>
          <div id="print-root" className="fixed inset-0 z-[10000] bg-white overflow-auto flex justify-center p-0">
            <div id="print-area" className="w-[210mm] min-h-[297mm] bg-white text-slate-900 relative p-[10mm] shadow-none">
              {(!previewInvoice.template || previewInvoice.template === 'template1' || !['template2', 'template3', 'alphesh'].includes(previewInvoice.template)) && (
                <InvoiceTemplate1 
...
function InvoiceRow({ inv, onCopy, onEdit, onDelete, onDownload, onPrint }) {
  return (
    <tr className="group hover:bg-indigo-50/30 transition-colors">
      ...
      <td className="px-6 py-4">
        <span className="text-sm font-bold text-gray-400">₹{Math.max(0, inv.totalAmount - inv.subtotal).toLocaleString()}</span>
      </td>
      <td className="px-6 py-4">
