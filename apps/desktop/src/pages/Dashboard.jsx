...
function Dashboard() {
  ...
  const { data: dashboardData, isLoading: isReportLoading } = useQuery({
  ...
  const totalExpenses = (stats?.totals?.purchases?.amount || 0) + (stats?.totals?.expenses?.amount || 0);
  const totalCollected = stats?.totalCash || 0;
  ...
  const chartData = useMemo(() => {
    ...
    invoices.forEach(inv => {
      const monthId = format(new Date(inv.date), 'yyyy-MM');
      const month = months.find(m => m.id === monthId);
      if (month) {
        month.revenue += (inv.totalAmount || 0);
      }
    });

    purchases.forEach(pur => {
      const monthId = format(new Date(pur.date), 'yyyy-MM');
      const month = months.find(m => m.id === monthId);
      if (month) {
        month.expense += (pur.totalAmount || 0);
      }
    });
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          amount={totalRevenue} 
          trend={`₹${chartData[5]?.revenue.toLocaleString()}`} 
          color="indigo" 
          icon={TrendingUp}
        />
        <StatCard 
          title="Monthly Expense" 
          amount={chartData[5]?.expense || 0} 
          trend={`Excl. Taxes`} 
          color="rose" 
          icon={TrendingDown}
        />
        ...
        <StatCard 
          title="Receivables" 
          amount={receivables} 
          trend={receivables > 10000 ? "Alert" : "Stable"} 
          color="amber" 
          icon={CreditCard}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="!p-8 border-none shadow-xl shadow-gray-200/50 flex flex-col">
          <h3 className="text-xl font-black text-gray-900 tracking-tight mb-8">Recent Activity</h3>
             ...
             {invoices.slice(0, 5).map((inv, i) => (
               <div key={inv.id} className="flex items-center gap-4 group cursor-pointer">
                  ...
                  <div className="flex-1 overflow-hidden">
                    ...
                    <p className="text-xs text-gray-400 font-medium truncate">{inv.party?.name || 'Walk-in'}</p>
                  </div>
          ...
          <button className="w-full py-4 text-xs font-bold text-gray-400 hover:text-indigo-600 transition-colors border-t border-gray-50 mt-8">
             Refresh Metrics
          </button>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <Card className="!p-6 border-none shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-lg font-black text-gray-900">{stats?.totals?.parties || 0}</p>
         ...
         <Card className="!p-6 border-none shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-lg font-black text-gray-900">{stats?.totals?.items || 0}</p>
         ...
         <Card className="!p-6 border-none shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-lg font-black text-gray-900">{growthRate}%</p>
         ...
         <Card className="lg:col-span-1 !p-1 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl shadow-xl shadow-indigo-500/20">
            <div className="bg-white/10 backdrop-blur-md rounded-[14px] p-5 text-white flex flex-col h-full">
               ...
               <p className="text-lg font-black mt-2">Artha Platinum</p>
               <button className="mt-auto w-full py-2 bg-white text-indigo-600 rounded-xl text-xs font-black shadow-lg">Manage</button>
            </div>
         </Card>
      </div>
    </div>
