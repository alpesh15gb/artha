...
export function InvoiceTemplate3({ invoice, business, party, items, totals = {} }) {
  ...
  const outstandingBalance = party?.balance || { previous: 0, current: total, total: total };

  return (
    <div className="w-full bg-white p-2 min-h-[1160px] font-sans text-xs border-2 border-black m-2 text-slate-900">
      <div className="flex justify-between items-center px-4 py-2 border-b-2 border-black">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold">State Name : {business?.address?.state || business?.state || '-'} Code : {business?.settings?.stateCode || business?.stateCode || '-'}</p>
      ...
      <table className="w-full border-b-2 border-black text-center table-fixed">
        ...
        <tbody className="selection:bg-indigo-50">
          {items.map((item, idx) => (
            <tr key={idx} className="border-b border-black last:border-b-0 min-h-[25px]">
              ...
              <td className="border-r-2 border-black text-left px-4 pt-1">
                 <p className="font-black text-[12px] uppercase">{item.item?.name || item.description}</p>
                 {item.item?.name && item.description && item.description !== item.item.name && (
                   <div className="text-[10px] normal-case opacity-60 italic leading-tight mt-0.5">{item.description}</div>
                 )}
          ...
          <tr className="bg-gray-100 font-black border-t-2 border-black">
             ...
             <td className="border-r-2 border-black"></td>
             <td className="border-r-2 border-black"></td>
             <td className="text-right px-1 py-2 font-black italic">{(totals?.subtotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="grid grid-cols-5 divide-x-2 divide-black border-b-2 border-black bg-white">
           ...
           <table className="w-full text-center text-[10px] font-bold">
              ...
              <tbody>
                 ...
                 <tr className="font-black bg-gray-50 uppercase">
                    ...
                    <td className="border-r border-black"></td><td className="border-r border-black">{(totals?.cgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                    <td className="border-r border-black"></td><td>{(totals?.sgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                 </tr>
              </tbody>
           </table>
        </div>
        <div className="col-span-2 flex flex-col justify-between font-black uppercase p-2">
           <div className="flex justify-between items-center text-sm">
              ...
              <span className="underline">{(totals?.subtotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
           </div>
           <div className="flex justify-between items-center text-[11px]">
              <span className="opacity-60 pl-8">CGST</span>
              <span>{(totals?.cgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
           </div>
           <div className="flex justify-between items-center text-[11px]">
              <span className="opacity-60 pl-8">SGST</span>
              <span>{(totals?.sgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
           </div>
        </div>
      </div>
