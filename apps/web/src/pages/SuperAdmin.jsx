...
function SuperAdmin() {
  ...
  const { data: acquisitionTrends } = useQuery({
    queryKey: ["admin-acquisition-trend"],
    queryFn: () => api.get("/admin/acquisition-trend").then((r) => r.data),
  ...
  const acquisitionData = acquisitionTrends?.data || [
    { name: "Mon", users: 0 },
    { name: "Tue", users: 0 },
    { name: "Wed", users: 0 },
    { name: "Thu", users: 0 },
    ...
    <div className="space-y-8 max-w-[1400px] mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-gray-100 p-1.5 rounded-2xl gap-1 shadow-inner">
          {["overview", "users", "subscriptions", "plans", "logs"].map(
            (tab) => (
              <button
                key={tab}
              ...
              <Card className="p-8 border-none shadow-2xl">
                ...
                <div className="space-y-6">
                  {users.slice(0, 5).map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-indigo-100 transition-all group"
                    >
                    ...
                    >
                      <td className="px-8 py-6">
                        <p className="font-black text-gray-900 text-base">
                          {user.name || "Set Profile"}
                        </p>
                        <p className="text-xs text-indigo-500 font-bold tracking-tight">
                          {user.email}
                        </p>
                      </td>
                      <td className="px-8 py-6">
                        <p className="font-bold text-gray-600">
                          {user._count.businesses} Business Account(s)
                        </p>
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                confirm(
                                  `CRITICAL: This will permanently delete ${user.email} and ALL their business data. This cannot be undone. Proceed?`,
                                )
                              ) {
        ...
        {activeTab === "plans" && (
          ...
          >
            {plans.map((plan) => (
              <Card
                key={plan.id}
                className="p-8 border-none shadow-2xl relative overflow-hidden group"
              >
                <div
                  className={cn(
                  ...
                  {logsData?.data?.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <Badge
                          variant="default"
                          className="bg-indigo-50 text-indigo-600 border-none font-black uppercase text-[10px]"
                        >
                          {log.action}
                        </Badge>
                      </td>
