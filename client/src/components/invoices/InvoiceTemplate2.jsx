...
export function InvoiceTemplate2({ invoice, business, party, items, totals }) {
  return (
    <div className="w-full bg-white p-6 min-h-[1160px] font-sans text-xs border border-gray-400">
      ...
      <table className="w-full border border-black text-center table-fixed">
        ...
        <tbody className="min-h-[400px]">
          {items.map((item, idx) => (
            <tr key={idx} className="border-b border-black">
              ...
              <td className="border-r border-black text-left px-2 py-2">
                <p className="font-bold whitespace-pre-wrap">{item.description || item.item?.name}</p>
          ...
          <tr className="bg-gray-50 font-bold border-t border-black">
            ...
            <td className="border-r border-black"></td>
            <td className="border-r border-black"></td>
            <td className="text-right px-2 text-sm italic py-1">₹ {totals.total.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table className="w-full border border-black border-t-0 text-center text-[10px]">
        <thead className="font-bold">
          ...
          <tr className="border-b border-black">
             <th className="border-r border-black py-0.5">{totals.subtotal.toFixed(2)}</th>
             <th className="border-r border-black">Rate</th>
             <th className="border-r border-black">Amount</th>
             <th className="border-r border-black">Rate</th>
             <th className="border-r border-black">Amount</th>
             <th className="font-bold">{(totals.cgst + totals.sgst).toFixed(2)}</th>
          </tr>
        </thead>
        <tbody className="font-bold">
          <tr>
            <td className="border-r border-black py-1">Total: {totals.subtotal.toFixed(2)}</td>
            <td className="border-r border-black"></td>
            <td className="border-r border-black">{totals.cgst.toFixed(2)}</td>
            <td className="border-r border-black"></td>
            <td className="border-r border-black">{totals.sgst.toFixed(2)}</td>
            <td className="">{(totals.cgst + totals.sgst).toFixed(2)}</td>
          </tr>
        </tbody>
      </table>
