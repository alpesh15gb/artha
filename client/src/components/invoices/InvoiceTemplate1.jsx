...
export function InvoiceTemplate1({ invoice, business, party, items, totals }) {
  const taxSummary = items.reduce((acc, item) => {
    const rate = item.taxRate || 0;
    if (!acc[rate]) acc[rate] = { taxable: 0, tax: 0 };
    acc[rate].taxable += (item.quantity * item.rate);
    acc[rate].tax += ((item.quantity * item.rate) * rate / 100);
    return acc;
  }, {});

  return (
    <div className="w-full bg-white p-4 min-h-[1160px] font-serif text-[11px] leading-tight border border-black selection:bg-gray-100">
      {/* Header */}
        ...
        <div className="grid grid-cols-2">
          <div className="border-r border-black">
            ...
            <div className="grid grid-cols-2">
              ...
              <div className="p-1 px-2 font-bold">{business?.state || 'Gujarat'}, State Code: {business?.stateCode || '24'}</div>
            </div>
          </div>
          <div>
            <div className="grid grid-cols-2 border-b border-black">
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx} className="border-t border-black min-h-[20px]">
              ...
              <td className="border-r border-black px-1 pt-1 font-bold">{item.description || item.item?.name}</td>
          ...
          <tr className="border-y border-black bg-gray-50 font-bold">
            ...
            <td className="border-r border-black"></td>
            <td className="border-r border-black"></td>
            <td className="text-right px-1">{totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="grid grid-cols-2 border-x border-b border-black">
        ...
        <div className="border-t border-black flex flex-col justify-between">
          <div className="divide-y divide-black border-b border-black">
            <div className="flex justify-between px-1 py-0.5"><span>CGST 9.0%</span><span>{(totals?.cgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
            <div className="flex justify-between px-1 py-0.5"><span>SGST 9.0%</span><span>{(totals?.sgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
            <div className="flex justify-between px-1 py-0.5 font-bold bg-gray-100 italic"><span>Net Payable</span><span className="text-lg">₹ {(totals?.total || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
          </div>
          <div className="p-2 flex gap-4 items-center">
            <div className="text-[10px] w-full">
              <div className="font-mono text-[7px] break-all">IRN: {invoice?.irn || 'D3E97BB23242FF072F7FDAE19EC8BE2EB78C983 F3B91'}</div>
            ...
            {business?.upiId && (
               <div className="h-16 w-16 bg-black flex items-center justify-center text-white text-[8px] text-center font-bold p-1 leading-[1]">e-invoice<br/>QR CODE</div>
            )}
          </div>
        </div>
      </div>
      ...
      <table className="w-full border-x border-b border-black text-center mt-4">
        <thead>
          ...
          <tr className="bg-gray-50 border-b border-black text-[9px]">
             <th className="border-r border-black font-normal py-0.5">₹ {totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</th>
             <th className="border-r border-black font-normal">Rate</th>
             <th className="border-r border-black font-normal">Amount</th>
             <th className="border-r border-black font-normal">Rate</th>
             <th className="border-r border-black font-normal">Amount</th>
             <th className="font-normal font-bold">₹ {(totals.cgst + totals.sgst).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</th>
          </tr>
        </thead>
        <tbody>
          <tr className="font-bold">
            <td className="border-r border-black py-0.5 px-2 text-right">Total: {totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
            <td className="border-r border-black"></td>
            <td className="border-r border-black px-2 text-right">{totals.cgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
            <td className="border-r border-black"></td>
            <td className="border-r border-black px-2 text-right">{totals.sgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
            <td className="px-2 text-right">{(totals.cgst + totals.sgst).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
          </tr>
        </tbody>
      </table>
