  ...
  try {
    ...
    const [
      ...
      prisma.invoice.aggregate({
        where: { businessId, status: { in: ['PAID', 'PARTIAL', 'SENT', 'DRAFT'] } },
        _sum: { totalAmount: true },
        _count: true,
      }),
      prisma.purchase.aggregate({
        where: { businessId, status: { in: ['PAID', 'PARTIAL', 'RECEIVED'] } },
        _sum: { totalAmount: true },
        _count: true,
      }),
    ...
    res.json({
      ...
      data: {
        totals: {
          invoices: { count: totalInvoices._count, amount: totalInvoices._sum.totalAmount || 0 },
          purchases: { count: totalPurchases._count, amount: totalPurchases._sum.totalAmount || 0 },
          expenses: { count: totalExpenses._count, amount: totalExpenses._sum.totalAmount || 0 },
...
router.get('/business/:businessId/party-ledger', async (req, res, next) => {
  try {
    ...
    const [transactions, invoices, purchases] = await Promise.all([
      ...
      prisma.invoice.findMany({
        where: { businessId, partyId, ...(fromDate && { date: { gte: new Date(fromDate) } }), ...(toDate && { date: { lte: new Date(toDate) } }) },
        orderBy: { date: 'asc' },
      }),
    ...
    const rawEntries = [
      ...transactions.map(t => ({
        ...
        type: t.type,
        description: t.narration || t.reference || (t.type === 'RECEIPT' ? 'Payment Received' : 'Payment Made'),
...
router.get('/business/:businessId/gst-summary', async (req, res, next) => {
  try {
    ...
    const [invoices, purchases] = await Promise.all([
      prisma.invoice.findMany({
        where: { businessId, status: { in: ['PAID', 'PARTIAL', 'SENT', 'DRAFT'] }, ...dateFilter },
        include: { party: true },
      }),
      prisma.purchase.findMany({
        where: { businessId, status: { in: ['PAID', 'PARTIAL', 'RECEIVED'] }, ...dateFilter },
        include: { party: true },
      }),
    ]);

    const salesSummary = invoices.reduce((acc, inv) => ({
      ...
      totalTax: acc.totalTax + inv.cgstAmount + inv.sgstAmount + inv.igstAmount + inv.cessAmount,
    }), { totalSales: 0, totalTaxableSales: 0, cgstCollected: 0, sgstCollected: 0, igstCollected: 0, cessCollected: 0, invoiceCount: 0, totalTax: 0 });

    const purchaseSummary = purchases.reduce((acc, pur) => ({
      ...
      totalTax: acc.totalTax + pur.cgstAmount + pur.sgstAmount + pur.igstAmount + pur.cessAmount,
    }), { totalPurchases: 0, totalTaxablePurchases: 0, cgstPaid: 0, sgstPaid: 0, igstPaid: 0, cessPaid: 0, purchaseCount: 0, totalTax: 0 });

    const b2bInvoices = invoices.filter(inv => inv.party?.gstin);
    const b2cInvoices = invoices.filter(inv => !inv.party?.gstin);
    const b2bLarge = b2cInvoices.filter(inv => inv.totalAmount >= 250000);
    ...
    for (const inv of invoices) {
      ...
      for (const item of items) {
        ...
        if (!hsnSummary[hsn]) {
          hsnSummary[hsn] = { hsnCode: hsn, taxableAmount: 0, cgstAmount: 0, sgstAmount: 0, igstAmount: 0, cessAmount: 0, quantity: 0 };
        }
...
router.get('/business/:businessId/profit-loss', async (req, res, next) => {
  try {
    ...
    const totalSalesTax = invoices.reduce((sum, inv) => sum + inv.cgstAmount + inv.sgstAmount + inv.igstAmount, 0);
    const totalSalesDiscount = invoices.reduce((sum, inv) => sum + inv.discountAmount, 0);
    const totalPurchases = purchases.reduce((sum, pur) => sum + pur.subtotal, 0);
    const totalPurchaseTax = purchases.reduce((sum, pur) => sum + pur.cgstAmount + pur.sgstAmount + pur.igstAmount, 0);
    const totalExpenses = expenses.reduce((sum, exp) => sum + exp.totalAmount, 0);
    const totalExpenseTax = expenses.reduce((sum, exp) => sum + exp.taxAmount, 0);
...
router.get('/business/:businessId/party-balance-summary', async (req, res, next) => {
  try {
    ...
    const parties = await prisma.party.findMany({
      ...
      include: {
        invoices: {
          where: { status: { in: ['PAID', 'PARTIAL', 'SENT', 'DRAFT'] } },
        },
        purchases: {
          where: { status: { in: ['PAID', 'PARTIAL', 'RECEIVED'] } },
        },
        transactions: true,
      },
    });
    ...
    const summary = parties.map(party => {
      const totalInvoiced = party.invoices.reduce((sum, inv) => sum + inv.totalAmount, 0);
      const totalInvoicePaid = party.invoices.reduce((sum, inv) => sum + inv.paidAmount, 0);
      const totalPurchased = party.purchases.reduce((sum, pur) => sum + pur.totalAmount, 0);
      const totalPurchasePaid = party.purchases.reduce((sum, pur) => sum + pur.paidAmount, 0);
      const receipts = party.transactions
        .filter(t => t.type === 'RECEIPT' && t.partyId === party.id)
        .reduce((sum, t) => sum + t.amount, 0);
      const payments = party.transactions
        .filter(t => t.type === 'PAYMENT' && t.partyId === party.id)
        .reduce((sum, t) => sum + t.amount, 0);

      const receivable = party.balanceType === 'RECEIVABLE';
      const totalBilled = receivable ? totalInvoiced : totalPurchased;
      const totalPaid = receivable ? totalInvoicePaid + receipts : totalPurchasePaid + payments;
      const balance = party.openingBalance + totalBilled - totalPaid;
    ...
    const totals = summary.reduce(
      (acc, p) => ({
        receivable: acc.receivable + (p.balanceType === 'RECEIVABLE' ? p.closingBalance : 0),
        payable: acc.payable + (p.balanceType === 'PAYABLE' ? p.closingBalance : 0),
  ...
  try {
    ...
    const [
      ...
      prisma.party.findMany({
        ...
        include: {
          invoices: { where: { status: { in: ['PAID', 'PARTIAL', 'SENT', 'DRAFT'] }, ...dateFilter } },
          purchases: { where: { status: { in: ['PAID', 'PARTIAL', 'RECEIVED'] }, ...dateFilter } },
    ...
    const sales = invoices.reduce((sum, inv) => sum + inv.subtotal, 0);
    const salesReturns = 0;
    const purchasesTotal = purchases.reduce((sum, pur) => sum + pur.subtotal, 0);
    const expensesTotal = expenses.reduce((sum, exp) => sum + exp.totalAmount, 0);
    ...
    const totalCredits = sales + sundryCreditors + advancesFromCustomers + netProfit;
    ...
    res.json({
      ...
      data: {
        ...
        ledgerAccounts: [
          ...
          { name: 'Sales Return', type: 'expense', debit: salesReturns, credit: 0 },
          { name: 'Purchases Account', type: 'expense', debit: purchasesTotal, credit: 0 },
          { name: 'Direct Expenses', type: 'expense', debit: directExpenses, credit: 0 },
          { name: 'Indirect Expenses', type: 'expense', debit: indirectExpenses, credit: 0 },
          { name: 'Sundry Debtors', type: 'asset', debit: sundryDebtors, credit: 0 },
          { name: 'Sundry Creditors', type: 'liability', debit: 0, credit: sundryCreditors },
          { name: 'Advances from Customers', type: 'liability', debit: 0, credit: advancesFromCustomers },
          { name: 'Advances to Suppliers', type: 'asset', debit: advancesToSuppliers, credit: 0 },
          { name: 'Bank Accounts', type: 'asset', debit: bankBalance, credit: 0 },
          { name: 'Cash in Hand', type: 'asset', debit: cashBalance, credit: 0 },
          { name: 'Net Profit', type: 'income', debit: 0, credit: netProfit },
        ],
        totals: {
          totalDebits,
          totalCredits,
...
router.get('/business/:businessId/day-book', async (req, res, next) => {
  try {
    ...
    const [invoices, purchases, expenses, transactions, receipts] = await Promise.all([
      prisma.invoice.findMany({
      ...
      prisma.transaction.findMany({
        where: { businessId, date: { gte: startOfDay, lte: endOfDay } },
        include: { party: { select: { name: true } }, bankAccount: { select: { bankName: true } }, cashAccount: { select: { name: true } } },
      }),
    ...
    const purchaseTotal = purchases.reduce((sum, pur) => sum + pur.totalAmount, 0);
    const expenseTotal = expenses.reduce((sum, exp) => sum + exp.totalAmount, 0);
    const receiptTotal = transactions.filter(t => t.type === 'RECEIPT').reduce((sum, t) => sum + t.amount, 0);
...
router.get('/business/:businessId/cash-book', async (req, res, next) => {
  try {
    ...
    let cashBalance = cashAccounts.reduce((sum, acc) => sum + acc.currentBalance, 0);
    let bankBalance = bankAccounts.reduce((sum, acc) => sum + acc.currentBalance, 0);

    const cashReceipts = cashTxns.filter(t => t.type === 'RECEIPT').reduce((sum, t) => sum + t.amount, 0);
    const cashPayments = cashTxns.filter(t => t.type === 'PAYMENT').reduce((sum, t) => sum + t.amount, 0);
    const bankReceipts = bankTxns.filter(t => t.type === 'RECEIPT').reduce((sum, t) => sum + t.amount, 0);
    const bankPayments = bankTxns.filter(t => t.type === 'PAYMENT').reduce((sum, t) => sum + t.amount, 0);
    ...
    res.json({
      ...
      data: {
        cashAccounts: cashAccounts.map(acc => ({
          ...acc,
          receipts: cashTxns.filter(t => t.cashAccountId === acc.id && t.type === 'RECEIPT').reduce((sum, t) => sum + t.amount, 0),
          payments: cashTxns.filter(t => t.cashAccountId === acc.id && t.type === 'PAYMENT').reduce((sum, t) => sum + t.amount, 0),
        })),
        bankAccounts: bankAccounts.map(acc => ({
          ...acc,
          receipts: bankTxns.filter(t => t.bankAccountId === acc.id && t.type === 'RECEIPT').reduce((sum, t) => sum + t.amount, 0),
          payments: bankTxns.filter(t => t.bankAccountId === acc.id && t.type === 'PAYMENT').reduce((sum, t) => sum + t.amount, 0),
        })),
        summary: {
          ...
          netCashFlow: cashReceipts + bankReceipts - cashPayments - bankPayments,
        },
        transactions,
      },
    });
...
router.get('/business/:businessId/account-statement/:accountId', async (req, res, next) => {
  try {
